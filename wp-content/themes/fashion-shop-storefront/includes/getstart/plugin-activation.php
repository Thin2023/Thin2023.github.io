...
if ( ! class_exists( 'Fashion_Shop_Storefront_Plugin_Activation_WPElemento_Importer' ) ) {
    ...
    class Fashion_Shop_Storefront_Plugin_Activation_WPElemento_Importer {
        ...
        public function __construct() {
            ...
            $actions                   = $this->fashion_shop_storefront_get_recommended_actions();
        ...
        public function fashion_shop_storefront_recommended_elemento_importer_plugins_array($plugins){
            $plugins[] = array(
        ...
        public function fashion_shop_storefront_get_recommended_actions() {
            ...
            $plugins = $this->fashion_shop_storefront_get_recommended_plugins();
            ...
            if ($plugins) {
                foreach ($plugins as $key => $plugin) {
                    ...
                    $link_and_is_done  = $this->fashion_shop_storefront_get_plugin_buttion($plugin['slug'], $plugin['name'], $plugin['function']);
        ...
        public function fashion_shop_storefront_get_recommended_plugins() {
            ...
            $plugins = apply_filters('fashion_shop_storefront_recommended_plugins', array());
            return $plugins;
        }
        ...
        public function fashion_shop_storefront_get_plugin_buttion($slug, $name, $function) {
                ...
                if (!$is_installed) {
                    ...
                    $button_html        = sprintf('<a class="fashion-shop-storefront-plugin-install install-now button-secondary button" data-slug="%1$s" href="%2$s" aria-label="%3$s" data-name="%4$s">%5$s</a>',
                        esc_attr($slug),
                    ...
                    $button_html = sprintf('<a class="fashion-shop-storefront-plugin-activate activate-now button-primary button" data-slug="%1$s" href="%2$s" aria-label="%3$s" data-name="%4$s">%5$s</a>',
                        esc_attr($slug),
                    ...
                    $button_html = sprintf('<div class="action-link button disabled"><span class="dashicons dashicons-yes"></span> %s</div>', esc_html__('Active', 'fashion-shop-storefront'));
                    $is_done     = true;
                }
